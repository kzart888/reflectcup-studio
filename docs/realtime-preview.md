...
| --- | ---: | --- | --- | --- |
| `warm-craftsman-home` | v4 | Interior concept option 1 | Meshopt GLB wooden table, sofa and potted plant plus an explicit metre-scale room shell and PBR oak floor | Solid visible background; Poly Haven Warm Restaurant 1K/2K HDR is PMREM/environment-lighting only |
| `forest-camp-evening` | v4 | Outdoor concept option 2 | Meshopt GLB outdoor table set with both source chairs hidden, lantern, off-axis tent at runtime scale `0.4`, and a matte near-ground receiver | Visible unblurred Poly Haven Dark Autumn Forest 1K HDR for Low/Medium, 2K for High; same source feeds PMREM |
...
The v4 compositions reuse the immutable v3 Poly Haven model payloads with embedded base-colour, tangent-space normal and packed AO/roughness/metallic inputs. The source glTF omits an explicit occlusion binding, so the runtime reuses the shared ARM texture's R channel as `aoMap` while G/B remain roughness/metallic inputs. The small Kenney tent is a stylised colour-only mid/far prop. `GLTFLoader` is configured with `MeshoptDecoder`; scene instances clone their owned geometries, materials and texture objects so a scene switch can dispose them without mutating the loader source.

Concept images under `docs/assets/scenes/concepts/` define art direction only. They are not presented as rotatable 360° backgrounds. The home v4 HDR is never drawn behind the explicit room shell; it is decoded only to build the oriented PMREM/environment response. Forest v4 draws its oriented HDR as the distant background and uses that same source for PMREM. The custom cup shader samples the resulting prefiltered cube-UV texture rather than the raw equirectangular texture. Because neither HDR was rendered from the same authored near/mid scene, exact prop/background/reflection correspondence is not claimed.
...
The home and forest v4 releases reuse byte-identical v3 `table-shadow.png` files rendered offline with Blender 5.2/Cycles from fixed area lights and the `curved-cup-v3` cup, handle and saucer shadow proxy. The 1024 × 768 RGBA decals provide continuous soft penumbrae without realtime shadow-map stair steps. The separate 1024 × 1024 Cycles-derived `curved-cup-v3` contact AO is profile-specific and clipped to the dish. The studio v2 scene intentionally retains its older analytic/Sharp decals for immutable replay.
