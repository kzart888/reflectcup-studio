...
| `forest-camp-evening` | Outdoor concept option 2 | Walnut table, ground, instanced trunks, tent, camp chair and lantern | Poly Haven Dark Autumn Forest HDR |
...
Each active HDR is rotated consistently for the visible background and environment lighting. The cup's custom mirror shader samples a PMREM generated from that same HDR, so rough reflection uses Three.js's prefiltered cube-UV path rather than the raw equirectangular texture. Mobile and the initial desktop pass use the 1K HDR; an eligible idle desktop promotes to the declared 2K high tier in a React transition. PMREM replacement retains the previous target until the new target is ready, so the cup does not disappear during promotion.
...
Rendering uses `frameloop="demand"`, ACES Filmic tone mapping and adaptive DPR. The mobile cap defaults to 1.5 and desktop to 2; interaction can temporarily regress DPR. A coarse-pointer device or `saveData` starts and stays at low quality. A regular desktop starts at medium, promotes to high while idle, and falls back to low if the performance monitor declines. Every tier retains the same objects and composition; tiers change map resolution, material detail and mesh tessellation only. A newly uploaded image uses a browser-local preview capped at 1024 px; a restored session receives a separate server-generated WebP sidecar capped at 2048 px. The full normalized source stays private for server rendering, and 4096 production images are never uploaded to the GPU.
