...
Customer photos are private. Uploads are size- and pixel-limited, decoded rather than trusted by extension, converted to sRGB, re-encoded without EXIF/GPS and served only through an authorized session. Storage paths are never public identifiers.
...
Replacing a draft image atomically detaches prior source and preview records. Every corresponding binary deletion writes a tombstone in the same database transaction; a storage worker claims tombstones with a lease and retries failures with backoff. Confirmed snapshot assets remain immutable and are never handled by this cleanup path.
