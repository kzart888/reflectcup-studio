...
Required checks are lint, TypeScript, unit, PostgreSQL integration, production-worker build, production build, dependency audit and Playwright desktop/mobile flows. CI regenerates the immutable nominal profile and calibration fixtures, rejects a dirty diff, and builds the production container. The shipped `curved-cup-v3` optical manifest and current mixed-version scene catalog (home/forest v4, studio v2) are independently checksum-tested; optical tests pin every historical v2 byte, while scene tests hash every referenced runtime asset, canonicalize the visual/render release contract, pin legacy v1 and former home/forest v2/v3 identities, and pin each current version/checksum. CI regeneration of `curved-cup-v3`, the historical `pnpm scene:bake` outputs and the Blender-version-pinned v3 Cycles lighting payload reused by scene v4 with dirty-diff checks remains release-hardening work.
...
- Scene/catalog tests: server/client IDs align with home/forest v4 and studio v2; canonical checksums match the pinned current values while legacy v1/v2/v3 identities remain unchanged; every declared public asset exists and matches its byte size/SHA-256; actual low/medium/high bytes remain within 4/7/12 MB; generated table-shadow and cup-contact files exist. Runtime-contract/layout tests also pin home v4 to a solid background and explicit room shell, forest v4 to an unblurred environment background plus matte receiver with no procedural trunks, and floor-relative prop placement/table orientation.
...
The current GLB/PBR near/mid geometry plus third-party HDR composition must not be accepted as the completed KTX2/UV2/full-scene-bake/360-panorama pipeline. Before claiming that pipeline, add golden screenshots for each quality tier and require lighting/composition parity rather than merely asset availability.
